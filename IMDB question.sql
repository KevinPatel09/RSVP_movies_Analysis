...
SELECT 
	...
	SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END ) AS null_production_companies
FROM movie;


-- we can see countries, worlwide_gross_income, languages, production_companies columns has null values. 
...
-- Monthly trend look like zig-zag manner, not forming one sided graph, it's not either upside nor downside .
...
-- we can see Drama genre has highest number of movies 
...
INNER JOIN movie AS m 
ON g.movie_id = m.id 
WHERE m.country LIKE 'USA' AND MONTH(date_published) = 3 AND year = 2017 AND r.total_votes > 1000
GROUP BY g.genre 
ORDER BY movie_count DESC ;
...
SELECT 
	...
	SUM(CASE WHEN height IS NULL THEN 1 ELSE 0 END ) AS null_heights,
...
SELECT n.name AS director_name, COUNT(dm.movie_id) AS director_movie_count
...
WHERE avg_rating > 8
GROUP BY n.name
ORDER BY director_movie_count DESC
LIMIT 3;
...
INNER JOIN names AS n
ON rm.name_id = n.id
WHERE median_rating >= 8 AND category LIKE 'actor'
GROUP BY actor_name
ORDER BY movie_count DESC
LIMIT 2;
...
INNER JOIN names AS n
ON rm.name_id = n.id
WHERE category LIKE 'actor' AND country LIKE 'india'
GROUP BY name
HAVING COUNT(m.id)>=5
LIMIT 1; 
...
INNER JOIN names AS n
ON rm.name_id = n.id
WHERE category LIKE 'actress' AND country LIKE 'india' AND languages LIKE 'hindi'
GROUP BY name
HAVING COUNT(m.id)>=3
LIMIT 5; 
...
SELECT genre,
       ...
       AVG(ROUND(AVG(duration),2)) OVER (ORDER BY genre ROWS 10 PRECEDING) AS moving_total_duration
FROM movie AS m 
INNER JOIN genre as g
ON m.id = g.movie_id
GROUP BY genre
...
(
SELECT g.genre, COUNT(g.movie_id) AS nos_of_movies
FROM genre AS g
INNER JOIN movie AS m
ON g.movie_id = m.id
GROUP BY genre
ORDER BY nos_of_movies DESC
LIMIT 3
),
...
INNER JOIN genre AS g
	USING (movie_id)
WHERE avg_rating >8 AND category LIKE 'actress' AND genre LIKE 'drama'
GROUP BY name
LIMIT 3;
...
WITH movie_predict AS
(
SELECT dm.name_id, name, dm.movie_id, m.date_published,
...
date_diff AS 
( 
SELECT *,DATEDIFF(next_date, date_published) AS difference
FROM movie_predict
...
SELECT dm.name_id AS director_id, 
       ...
       ROUND(AVG(avg_rating),2) AS average_ratings,
       SUM(total_votes) AS total_votes,
       MIN(avg_rating) AS min_rating,
       MAX(avg_rating) AS max_rating,
       SUM(duration) AS total_duration,
       ROW_NUMBER() OVER(ORDER BY COUNT(movie_id) DESC) AS director_row_rank
...
INNER JOIN movie AS m
ON r.movie_id = m.id
INNER JOIN avg_date_diff AS ad
USING(name_id)
